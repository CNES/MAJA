...
class ConcatenateMaskImages : public Application
{
public:
  /** Standard class typedefs. */
  typedef ConcatenateMaskImages             Self;
  ...
  itkTypeMacro(ConcatenateMaskImages, otb::Application);

  /** Filters typedef */
  typedef otb::ImageList<UInt8ImageType> ImageListType;
  typedef otb::ImageListToVectorImageFilter<ImageListType, UInt8VectorImageType>                                ListConcatenerFilterType;
  typedef otb::MultiToMonoChannelExtractROI<UInt8VectorImageType::InternalPixelType, UInt8ImageType::PixelType> ExtractROIFilterType;
  ...
  void DoExecute() override
  {
    ...
    for (unsigned int i = 0; i < GetNumberOfElementsInParameterInputImageList("il"); i++)
    {
    	...
    	if (className == "VectorImage") {
    		UInt8VectorImageType * vectIm = dynamic_cast<UInt8VectorImageType*>(imgBase);
    		if (vectIm)
    		{
    			...
    			vnsExceptionDataMacro("Wrong type of input image, only uint8 supported")
    		}
    	} else {
    		UInt8ImageType * double_Im = dynamic_cast<UInt8ImageType*>(imgBase);
    		if (double_Im)
    		{
...
OTB_APPLICATION_EXPORT(vns::Wrapper::ConcatenateMaskImages)
