...
class L2InitProcessor(L2Processor):

    def __init__(self, apphandler):
        self._name = "L2_INIT"
        super(L2InitProcessor, self).__init__(apphandler)
        self._l1productfilename = ""
        self._l1product = None
        # Need only one plugin in Init mode
        self.plugin = None
    ...
    def scientific_processing(self):
        ...
        if l_GIPPL2COMMHandler.get_value_b("UseCamsData"):
            ...
            LOGGER.debug("Product validity date : "+self._l1product.UTCValidityStart + " : "+ self._l1product.UTCValidityStop)
            l_avgTime = date_utils.get_average_time(self._l1product.UTCValidityStart, self._l1product.UTCValidityStop)
        ...
        l1_image_reader = L1ImageReaderProvider.create(self._l1product.PluginName, self._l1product,
        ...
        # ---------------------------------------------------------------------------------------------
        # Initialize L2 image writer
        LOGGER.debug("Starting L2InitProcessor::PreProcessing(): Initialize the OutputL2ImageFileWriter ...")
        ...
        super(L2InitProcessor, self).process_one_product(l_InitMode, l_BackwardMode, l_enableL2resolution,
    ...
    def post_processing(self):
        ...
        LOGGER.info("Ending PostProcessing")
